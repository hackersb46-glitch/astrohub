...
async def advanced_function_run(req: AdvancedFunctionRunRequest) -> dict:
    ...
    try:
        ...
        mgr: "PTZDeviceController" = _managers.get("ptz_controller")  # type: ignore[assignment]
        username = req.username or "admin"
        password = req.password
        port = req.port or 80
        if not password:
        ...
        # v7.33: run_all 已通过 device_path 统一保存 function.json，无需重复写入
        ptz_mgr_fn: "PTZDeviceController" = _managers.get("ptz_controller")  # type: ignore[assignment]
...
async def advanced_function_status(device_ip: str = Query(default="")) -> dict:
    ...
    try:
        ...
        ptz_mgr: PTZDeviceController | None = _managers.get("ptz_controller")  # type: ignore[assignment]
...
async def advanced_limit_run(req: AdvancedLimitRunRequest) -> dict:
    ...
    try:
        ...
        ptz_mgr_lm: "PTZDeviceController" = _managers.get("ptz_controller")  # type: ignore[assignment]
        ...
        # v7.51: CSV 已通过 device_path 写入，JSON 摘要写入 mac_clean 目录
        mac_clean = mac.replace(':', '').lower() if mac else "unknown"
        config_saved = False
        try:
            store.save_limit_results(mac_clean, results)
            config_saved = True
        except Exception:
            pass
...
async def advanced_speed_run(req: AdvancedSpeedRunRequest) -> dict:
    ...
    try:
        from src.ptz.isapi.client import ISAPIClient
        from src.ptz.isapi.ptz import PTZDeviceController
        ...
        # 如果密码为空，尝试从PTZDeviceController获取
        if not password:
            mgr: PTZDeviceController | None = _managers.get("ptz_controller")  # type: ignore[assignment]
            if mgr:
        ...
        ptz = PTZDeviceController(client)
        ...
        # 先获取MAC
        ptz_mgr_sp: "PTZDeviceController" = _managers.get("ptz_controller")  # type: ignore[assignment]
        ...
        config_saved = False
        try:
            store.save_speed_results(mac_clean, results)
            config_saved = True
        except Exception:
            pass
    ...
    async def _run_detect():
        try:
            ...
            if "limit" in req.items:
                ...
                limit_result = await asyncio.to_thread(limit_tester.run_all_tests)
            ...
            if "speed" in req.items:
                ...
                speed_result = await asyncio.to_thread(speed_tester.run_all_tests, None, req.speed_profile)
...
async def restart_astrohub() -> dict:
    """v7.52: 重启 AstroHub 服务 - 立即启动新进程，不延迟."""
    import subprocess
    import os
    import platform
...
'''
        ...
        # Linux/macOS: 立即启动新进程
        subprocess.Popen(f'cd "{cwd}" && python src/main/main.py &', shell=True, cwd=cwd)
