...
class DeviceManager:
    ...
    def __init__(self, storage_path: Path | None = None) -> None:
        ...
        self._load()
        log.info(f"DeviceManager 初始化完成, devices_dir={self._devices_dir}")
    ...
    def _save(self) -> None:
        ...
        with self._lock:
            ...
            log.info(f"设备数据已保存: {len(self._devices)} 台设备")

    def _load(self) -> None:
        """v6.10: 从 devices/{mac}/info.json 加载设备数据。"""
        if not self._devices_dir.exists():
            log.info(f"设备目录不存在，初始化空状态: {self._devices_dir}")
            return
        ...
        try:
            ...
            for info_file in self._devices_dir.rglob("info.json"):
                try:
                    ...
                    log.warning(f"加载设备文件失败 {info_file}: {e}")
                    continue
            
            log.info(f"加载设备数据: {len(self._devices)} 台设备")
