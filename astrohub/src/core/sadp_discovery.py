...
class SADPManager:
    ...
    def discover_devices(self, timeout: float = 3.0, bind_ip: str = "0.0.0.0") -> list[dict]:
        """执行完整的设备发现流程.
        
        v7.15: 优化发现速度 - 发送探测后立即检查，发现设备立即返回
        
        Args:
            timeout: 最大等待时间 (秒)，默认 3秒
        ...
        try:
            # v7.15: 发送一次探测，立即检查响应
            self.send_inquiry()
            initial_count = len(self._devices)
            ...
            for i in range(max_checks):
                ...
                if current_count > initial_count:
                    print(f"[SADP] 发现 {current_count} 台设备 (耗时 {time.time() - start_time:.2f}s)")
                    time.sleep(0.3)  # 等待可能的更多设备
                    break
