...
def run_all_tests(ptz: Any, config: Any = None, speed_profile: str = 'lite') -> list[dict[str, Any]]:
    """运行完整速度测试。
    ...
    for _ in range(300):
        pos = ptz.get_position()
        if pos and abs(pos.get("pan", 0) - 1800) <= 10 and abs(pos.get("tilt", 0) - 450) <= 5:
            stable_count += 1
            if stable_count >= 20:
                break
        else:
...
class SpeedTester:
    ...
    def run_all_tests(self, config: Any = None, speed_profile: str = 'lite') -> list[dict[str, Any]]:
        return run_all_tests(self.ptz, config, speed_profile)
