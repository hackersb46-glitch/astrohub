...
from src.advanced.device_settings import save_settings as save_device_settings, load_settings
...
<FocusConfiguration version="2.0" xmlns="http://www.hikvision.com/ver20/XMLSchema">
  ...
  <focusLimited>300</focusLimited>
        ...
        if result.status_code == 200:
            ...
            # v8.47: 保存对焦模式到 settings.json
            try:
                mac_clean = device_id.replace(":", "").replace("-", "").lower()
                if len(mac_clean) == 12:
                    save_device_settings(mac_clean, {"focus": {"mode": req.mode}})
            except Exception:
                pass
...
<saturationLevel>{updated.get("saturation", 50)}</saturationLevel>
        ...
        if put_resp.status_code == 200:
            ...
            try:
                mac_clean = device_id.replace(":", "").replace("-", "").lower()
                if len(mac_clean) == 12:
                    save_device_settings(mac_clean, {"image_settings": updated})
            except Exception:
                pass
...
<WhiteBalanceBlue>{blue_val}</WhiteBalanceBlue>
        ...
        if put_resp.status_code == 200:
            ...
            # 保存设置到 settings.json (v8.45)
            try:
                mac_clean = device_id.replace(":", "").replace("-", "").lower()
                if len(mac_clean) == 12:
                    save_device_settings(mac_clean, {"whitebalance": {"mode": mode_val, "red_gain": red_val, "blue_gain": blue_val}})
            except Exception:
                pass
...
<InterFrameNoiseReduceLevel>{temporal}</InterFrameNoiseReduceLevel>
        ...
        if put_resp.status_code == 200:
            ...
            try:
                mac_clean = device_id.replace(":", "").replace("-", "").lower()
                if len(mac_clean) == 12:
                    save_device_settings(mac_clean, {"noisereduce": {"spatial_level": spatial, "temporal_level": temporal}})
            except Exception:
                pass
  ...
  <ExposureType>{mode}</ExposureType>
        ...
        if put_resp.status_code == 200:
            ...
            try:
                mac_clean = device_id.replace(":", "").replace("-", "").lower()
                if len(mac_clean) == 12:
                    save_device_settings(mac_clean, {"exposure": {"mode": mode}})
            except Exception:
                pass
  ...
  <ShutterLevel>{level}</ShutterLevel>
        ...
        if put_resp.status_code == 200:
            ...
            try:
                mac_clean = device_id.replace(":", "").replace("-", "").lower()
                if len(mac_clean) == 12:
                    save_device_settings(mac_clean, {"shutter": {"level": level}})
            except Exception:
                pass
  ...
  <IrisLevel>{level}</IrisLevel>
        ...
        if put_resp.status_code == 200:
            ...
            try:
                mac_clean = device_id.replace(":", "").replace("-", "").lower()
                if len(mac_clean) == 12:
                    save_device_settings(mac_clean, {"iris": {"level": level}})
            except Exception:
                pass
  ...
  <SharpnessLevel>{level}</SharpnessLevel>
        ...
        if put_resp.status_code == 200:
            ...
            try:
                mac_clean = device_id.replace(":", "").replace("-", "").lower()
                if len(mac_clean) == 12:
                    save_device_settings(mac_clean, {"sharpness": {"level": level}})
            except Exception:
                pass
...
async def get_filter_settings(device_id: str) -> dict:
    ...
    client = ctrl.client
    try:
        resp = await asyncio.to_thread(client.get, "/Image/channels/1")
        if resp.status_code != 200:
...
async def set_filter_settings(device_id: str, body: dict) -> dict:
    ...
    try:
        ...
        if ir_cut:
            xml_str = re.sub(
                r'(<IrcutFilterType>)[^<]*(</IrcutFilterType>)',
                f'\\g<1>{ir_cut}\\g<2>',
                xml_str
            )

        put_resp = await asyncio.to_thread(client.put, "/Image/channels/1", xml_str)
        if put_resp.status_code == 200:
    ...
    try:
        ...
        if put_resp.status_code == 200:
            ...
            try:
                mac_clean = device_id.replace(":", "").replace("-", "").lower()
                if len(mac_clean) == 12:
                    save_device_settings(mac_clean, {"slow_shutter": {"dss_level": dss_level, "enabled": dss_level != "off"}})
            except Exception:
                pass
...
@api_router.get("/ptz/{device_id}/image/settings-saved", summary="获取设备保存的设置", tags=["Image"])
async def get_saved_settings(device_id: str) -> dict:
    """获取设备保存的图像参数设置 (用于记忆恢复)。"""
    try:
        from src.advanced.device_settings import load_settings
        mac_clean = device_id.replace(":", "").replace("-", "").lower()
        if len(mac_clean) != 12:
            return {"success": False, "message": "无效的设备标识"}
        
        settings = load_settings(mac_clean)
        return {"success": True, "data": settings or {}}
    except Exception as e:
        return {"success": False, "message": f"获取保存设置失败: {e}"}
