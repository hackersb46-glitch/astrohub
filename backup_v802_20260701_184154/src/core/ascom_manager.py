...
class ASCOMManager:
    ...
    def __init__(self, platform: str = "windows") -> None:
        ...
        try:
            ...
            log.info(f"ASCOMManager initialized, platform={self._platform}")
        except Exception as e:
            log.error(f"ASCOM platform init failed: {e}")
            self._initialized = False
    ...
    def _init_windows_platform(self) -> None:
        ...
        try:
            ...
            log.info("Windows ASCOM platform (COM) ready")
        except ImportError:
            self._com_available = False
            log.warning("pywin32 not available, ASCOM COM features limited")
    ...
    def _init_alpaca_platform(self) -> None:
        ...
        try:
            ...
            log.info("Alpaca ASCOM platform (HTTP) ready")
        except ImportError:
            self._alpaca_available = False
            log.warning("requests not available, Alpaca features limited")
    ...
    def _connect_device(self, device_id: str, device_type: str) -> bool:
        ...
        with self._lock:
            if not self._initialized:
                log.error("ASCOM platform not initialized, cannot connect device")
                return False

            if device_id in self._devices:
                existing = self._devices[device_id]
                if existing.status == STATUS_CONNECTED:
                    log.info(f"Device already connected: {device_id}")
                    return True
                log.info(f"Reconnecting device: {device_id}")
            ...
            try:
                if self._platform == "windows" and self._com_available:
                    ...
                    log.info(f"Connecting via COM {device_type}: {device_id}")
                    ...
                    log.info(f"Connecting via Alpaca {device_type}: {device_id}")
                    ...
                    log.error("No ASCOM platform backend available (COM/Alpaca)")
                ...
                if device.status == STATUS_CONNECTED:
                    ...
                    log.info(f"Device connected: {device_type} - {device.name} ({device_id})")
                    return True
                else:
                    self._devices[device_id] = device
                    return False
                ...
                self._devices[device_id] = device
                log.error(f"Device connection failed [{device_type}] {device_id}: {e}")
                return False
    ...
    def _alpaca_find_device(self, device_id: str, device_type: str) -> int:
        ...
        log.warning(f"Alpaca device not found: {device_id} ({device_type}), using default number 0")
        return 0
    ...
    def _load_capabilities(self, device: ASCOMDevice) -> None:
        ...
        try:
            ...
            log.warning(f"Get device capabilities failed [{device.device_id}]: {e}")
            device.capabilities = {}
    ...
    def get_device_status(self, device_id: str) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if device is None:
                log.warning(f"Device status query failed, device not found: {device_id}")
                return {
                    "device_id": device_id,
    ...
    def get_capabilities(self, device_id: str) -> dict[str, Any]:
        ...
        with self._lock:
            device = self._devices.get(device_id)
            if device is None:
                log.warning(f"Device capabilities query failed, device not found: {device_id}")
                return {}

            if device.status != STATUS_CONNECTED:
                log.warning(f"Device not connected, cannot get capabilities: {device_id}")
                return {}

            try:
                return self._get_capabilities_internal(device)
            except Exception as e:
                log.error(f"Get device capabilities exception [{device_id}]: {e}")
                return {}
    ...
    def _get_capabilities_internal(self, device: ASCOMDevice) -> dict[str, Any]:
        ...
        try:
            ...
            log.warning(f"Get internal capabilities failed [{device.device_id}]: {e}")
    ...
    def disconnect(self, device_id: str) -> bool:
        ...
        with self._lock:
            device = self._devices.get(device_id)
            if device is None:
                log.warning(f"Disconnect device failed, device not found: {device_id}")
                return False

            if device.status == STATUS_DISCONNECTED:
                log.info(f"Device already disconnected: {device_id}")
                return True
            ...
            try:
                ...
                log.info(f"Device disconnected: {device.device_type} - {device.name} ({device_id})")
                return True

            except Exception as e:
                log.error(f"Disconnect device failed [{device_id}]: {e}")
                device.error_message = str(e)
                return False
    ...
    def disconnect_all(self) -> None:
        ...
        log.info("All ASCOM devices disconnected")
