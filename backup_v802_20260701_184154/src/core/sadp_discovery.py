...
def _find_sadp_dll() -> str:
    ...
    优先级:
    1. PyInstaller 打包模式: exe 同级 SDK/hik/Sadp.dll
    2. 开发模式: APP_DIR/SDK/hik/Sadp.dll
    3. src/core/Sadp.dll (旧位置回退)
    """
    import sys
    ...
    if hasattr(sys, '_MEIPASS'):
        ...
        exe_sdk = exe_parent / 'SDK' / 'hik' / 'Sadp.dll'
        if exe_sdk.exists():
            return str(exe_sdk)
        # 旧路径回退
    ...
    try:
        ...
        sdk_sadp = SDK_LIBS_DIR / "hik" / "Sadp.dll"
        if sdk_sadp.exists():
            return str(sdk_sadp)
    ...
    raise FileNotFoundError(
        ...
        f"SDK/hik/Sadp.dll, src/core/Sadp.dll. "
        "Please copy SDK DLLs to SDK/hik/ directory."
    )
...
class SADPManager:
    ...
    def discover_devices(self, timeout: float = 0, bind_ip: str = "0.0.0.0") -> list[dict]:
        """返回已发现的设备列表（即时返回，不等待）。
        
        v7.117: 简化发现逻辑 - SADP后台持续运行，直接返回已发现设备。
        ...
        print(f"[SADP] 返回 {len(all_devices)} 台已发现设备")
        return all_devices
