...
class FunctionDetector:
    ...
    def traverse_function(self, item_key: str) -> FunctionResult:
        ...
        try:
            ...
            if mode != "read_only":
                ...
                # v7.52: exposure_mode 分离处理，直接使用 test_values
                if mode in ("shutter", "iris"):
                    ...
                    # 字符串选项型：直接使用 test_values（ISAPI 定义的 ExposureType 可选值）
                    result.opt_values = cap_def.get("test_values", [])
                else:
                    min_range, max_range = self._parse_range(root, cap_def["test_key"])
        ...
        if mode == "read_only":
            ...
            # 枚举型 (DSS: 慢快门倍率) - 直接使用 test_values
            result.opt_values = cap_def.get("test_values", [])
            # 枚举型 (DSS: 慢快门倍率) - 需要先启用再测试
            test_values = cap_def.get("test_values", [])
            original = self._original_xml.get(item_key, "")
            
            # DSS特殊处理：如果enabled=false，先设置为true
            import re
            if '<enabled>false</enabled>' in original.lower():
                enable_xml = re.sub(
                    r'(<enabled>)[^<]*(</enabled>)',
                    r'\g<1>true\g<2>',
                    original,
                ...
                self.client.put(endpoint, enable_xml)
    ...
    def run_all(self) -> dict[str, Any]:
        ...
        # 步骤1: 设置预置点10为HOME (1800,450,10)
        ptz = PTZController(self.client)
        ptz.setup_home_preset()
        print('Function起点: HOME preset 10 set')
...
# --- 活跃测试器注册（用于取消） ---
_active_func_detector: FunctionDetector | None = None


def cancel_active_function(device_ip: str) -> bool:
    """取消指定设备的功能探测。"""
    global _active_func_detector
    if _active_func_detector is not None:
        _active_func_detector.cancel()
        return True
    return False
