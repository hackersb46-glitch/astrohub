"""v8.11 E2E: Playwright 真实 UI 测试
流程:
  A. 白平衡: 自动(官方) → 验证按钮拦截 → 切手动 → 框选 → 迭代搜索
  B. 对焦:   自动 → 验证按钮拦截 → 切手动 → 框选 → 迭代搜索
"""
import time
...
def wait_toast(page, keyword, timeout_s=90):
    """等待包含关键字的 toast，适配迭代搜索较长时间。"""
...
def check_log(page, keyword, timeout_s=90):
    """检查操作日志区域是否包含关键字。"""
    for i in range(timeout_s * 2):
        time.sleep(0.5)
        logs = page.query_selector_all("#operationLogBox div")
        for log in logs:
            t = log.text_content() or ""
            if keyword in t:
                return True, t
...
def run():
    ...
    with sync_playwright() as p:
        ...
        page.click("#btnRegionWB")
        time.sleep(0.3)
        ok, captured = wait_toast(page, "手动", 5)
        ...
        check("拦截toast提示", ok or "手动" in captured,
              f"toast='{captured[:60]}'")

        # A3: 切到手动模式
        print("\n[A3] 切白平衡到手動...")
        page.select_option("#wbMode", "manual")
        ok, msg = wait_toast(page, "白平衡", 10)
        ...
        ok, msg = wait_toast(page, "白平衡完成", 120)
        check("白平衡迭代完成", ok, msg[:100])

        # 验证操作日志
        has_start = check_log(page, "白平衡迭代开始", 5)
        check("操作日志: 白平衡开始", has_start)
        has_step = check_log(page, "粗调", 2)
        check("操作日志: 包含迭代步", has_step)
        has_cleanup = check_log(page, "清理:", 3)
        check("操作日志: 清理完成", has_cleanup)
        ...
        ok, msg = wait_toast(page, "对焦完成", 120)
        check("对焦迭代完成", ok, msg[:100])

        # 验证操作日志
        has_focus_start = check_log(page, "对焦迭代开始", 5)
        check("操作日志: 对焦开始", has_focus_start)
        has_focus_cleanup = check_log(page, "清理:", 3)
        check("操作日志: 清理完成", has_focus_cleanup)
